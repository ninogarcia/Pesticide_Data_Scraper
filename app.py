...
# Streamlit app
def streamlit_app():
    st.title("ICAMA Pesticide Registration Crawler")
    
    search_term = st.text_input("Enter search term:", "Dimethoate")
    ...
    if st.button("Start Crawling"):
        ...
        async def progress_callback(message):
            ...
            # You might want to update the progress bar here based on some metric
        
        results = run_async(main(search_term, progress_callback))
        
        st.write(f"Total items scraped: {len(results)}")
        st.json(results)
